...
BEGIN
	...
	-- Insert stats from the gtfs_stop_times table.
	INSERT INTO my_gtfs_trip_stats (trip_id, direction_id, depart_time,arrive_time,first_stop_id,last_stop_id) SELECT 
			a.trip_id,
			a.direction_id,
	...
	-- Populate trip distances using gtfs_shapes distances (rounded to nearest 100m)
	UPDATE my_gtfs_trip_stats 
		SET trip_distance_km = trip_distances.trip_distance_km
	FROM (
			SELECT
				trip_id,
				round(CAST(MAX(shape_dist_traveled)/1000 AS NUMERIC),1)  AS trip_distance_km
			FROM gtfs_shapes
				LEFT JOIN gtfs_trips ON gtfs_trips.shape_id = gtfs_shapes.shape_id
			GROUP BY trip_id
		) AS trip_distances
	WHERE trip_distances.trip_id = my_gtfs_trip_stats.trip_id;

	--Populate days per week from "gtfs_calendar"
	-- NB: If calendar not populated refer populate_calendar_weekdays_from_calendar_dates.sql
	UPDATE my_gtfs_trip_stats
		SET day_period = CASE WHEN monday+tuesday+wednesday+thursday+friday >= 1 THEN 'MF' WHEN saturday =1 THEN 'Sat' WHEN sunday = 1 THEN 'Sun' ELSE NULL END,
		days_per_week = monday+tuesday+wednesday+thursday+friday+saturday+sunday
	FROM gtfs_calendar, gtfs_trips
	WHERE 
		my_gtfs_trip_stats.trip_id = gtfs_trips.trip_id
		AND gtfs_trips.service_id = gtfs_calendar.service_id; 
...
Usage:
	SELECT my_gtfs_generate_table_my_gtfs_route_catchment('SELECT row_number() OVER()::integer AS id, stop_id, ST_Buffer(ST_Transform(the_geom,2193), 400) AS the_geom FROM gtfs_stops');
...
CREATE OR REPLACE FUNCTION my_gtfs_generate_table_my_gtfs_route_catchment(stopCatchmentSql text DEFAULT 'SELECT row_number() OVER()::integer AS id, stop_id, ST_Buffer(ST_Transform(the_geom,2193), 400) AS the_geom FROM gtfs_stops')
  RETURNS text AS
$BODY$
DECLARE
	-- Nothing to declare
